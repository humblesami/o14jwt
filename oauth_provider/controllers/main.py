    ...
    def auth(self, **kw):
        ...
        if user.login == 'public':
            ...
            params = {'mode': 'login',
                      ...
                      'redirect': '/oauth2/auth?%s' % urlencode(kw)
                      }
            url = '/web/login'
            if 'trial' in scope.split(' '):
                url = '/web/signup'
            return self._response({'Location': '{url}?{params}'.format(url=url, params=urlencode(params))}, None, 302)
        else:
